...
<html>
	<head>
		<title>iBus | Add Accomodation</title>
		<meta charset="utf-8" />
...
xmlhttp.open("GET","insert_data.php?name="+name+"&capacity="+capacity+"&route="+route+"&key=acc",true);
xmlhttp.send();
}

function delete_acc(str)
{
	var result=confirm("Do you really want to delete this Accomodation?");
if(result)
{
if (window.XMLHttpRequest)
...
xmlhttp.onreadystatechange=function()
  ...
  if (xmlhttp.readyState==4 && xmlhttp.status==200)
    {
   alert('Accomodation Deleted Successfully');
   document.getElementById("delresult").innerHTML = xmlhttp.responseText;
    }
  }
xmlhttp.open("GET","insert_data.php?busid="+str+"&key=delacc",true);
xmlhttp.send();
}
}
        </script>
